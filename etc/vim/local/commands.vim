...
command! -bar FuzzyOpen call FugitiveDetect() | execute (!empty(FugitiveGitDir()) ? 'FzfGFiles! --cached --others --exclude-standard' : 'FZF!')
...
command! -range=% -nargs=1 -bar F  call <SID>FilterLines(<line1>, <line2>, '-P', <q-args>)
command! -range=% -nargs=1 -bar FV call <SID>FilterLines(<line1>, <line2>, '-Pv', <q-args>)
