...
fu! s:ConstructAndExecuteCmd(direction, commitCount, extraArgs, filePath, range) "{{{
    if a:range == [] "no range, setup and execute the command
        ...
        let cmd .= " --no-color --decorate=full --date=short --pretty=format:\"%d %s__SEP__%ar__SEP__%an__SEP__[%h]\" --graph -"
...
fu! s:GetFinalOutputForHashes(hashes) "{{{
    if len(a:hashes) > 0
        ...
        let cmd      .='--no-color --decorate=full --date=short --pretty=format:"%d %s__SEP__%ar__SEP__%an__SEP__[%h]%n" --graph -1 ${hash}; '
        let cmd      .= 'done'
