...
call s:command("-nargs=? -complete=customlist,s:CommitComplete Gcommit :execute s:Commit('<mods>', <q-args>) | wincmd T")
...
function! s:Commit(mods, args, ...) abort
  ...
  try
    ...
    try
      ...
      let command .= s:UserCommand() . ' commit -v ' . args
      if &shell =~# 'csh'
...
function! s:Grep(cmd,bang,arg) abort
  ...
  try
    ...
    exe 'silent! '.a:cmd.'! '.escape(s:ShellExpand(matchstr(a:arg, '\v\C.{-}%($|[''" ]\@=\|)@=')), '|#%')
