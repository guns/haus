...
module NERV::CLI::ReplHelpers
  ...
  def slurp path
    if path =~ %r{\A\w+://}
      IO.popen(['curl', '--user-agent', 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/46.0.2490.80 Safari/537.36', '--progress-bar', '--location', path]) { |io| io.read }
    else
      File.read path
    end
  end
